...
**Trustworthy agent-written data pipelines:** Running code ≠ correct code. AI is non-deterministic (same prompt yields different implementations) and can't see data/metadata (schema, units, quality flags), so it guesses silently. The antidote is a four-discipline framework: (1) **data inspection via MCP** — give the agent eyes on schema, samples, null rates before writing the transform; (2) **data contracts** — formal executable specs of the output (row semantics, column transforms, KPIs, invariants), replacing vague prompts; (3) **write-audit-publish gates** — stage → validate against contract → publish only if audit passes, failing loudly rather than loading wrong numbers; (4) **encoded discipline** — package all three into reusable skills/AGENTS.md so every new pipeline inherits the practice. A fifth element, the **correctness layer**, separates probabilistic agent work (reading intent, drafting SQL) from deterministic validation (parsing, lineage, equivalence checking via a compiler-like core), reducing token exposure and ensuring factual correctness. **Local vs. cloud data** is the final tradeoff: use local (DuckDB) for agent unit tests and verification (fast, sub-second feedback loops, no network), cloud (Snowflake, BigQuery) for production scale and real-time dashboards. This hybrid approach lets agents iterate rapidly on local verification before deploying to production.

Subtopics being tracked: `llm`, `world-models`, `robotics`, `agents`, `ai-infra`, `etl`, `pipelines`, `streaming`, `query-optimization`, `lakehouse`, `prompt-engineering`, `prediction-markets`, `correctness`, `data-contracts`, `testing`
...
| Data contract discipline → agent correctness | data-n-ai | The four-discipline framework (data inspection, contracts, write-audit-publish, skills) + deterministic correctness layer ensure agent-written pipelines are trustworthy. Applies to all agent-generated data code: ingestion, ETL, marts, everything. |
| Local verification → fast agent loops | data-n-ai | Agents iterate faster on local data (DuckDB, sub-second tests) than cloud data (multi-second to minute cloud queries). Hybrid approach (local + cloud) is the scalable pattern for agent DE workflows. |
